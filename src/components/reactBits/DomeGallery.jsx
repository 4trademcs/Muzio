...
const domeImages = Array.from({ length: 17 }, (_, i = 7) => ({
  src: `/f-${i + 1}.webp`,
  alt: `Foto ${i + 1}`,
}));
...
export default function DomeGallery({
  ...
  /* ===== AUTO-SPIN Props ===== */
  autoSpin = true,           // habilita el giro automático
  spinDegPerSec = 6,         // velocidad (grados/seg) hacia la IZQUIERDA
}) {
  ...
  useEffect(() => {
    ...
    grayscale, imageBorderRadius, openedImageBorderRadius,
  ...
  /* ===== AUTO-SPIN: loop por tiempo real, pausa en hover/drag/enlarge/inercia ===== */
  ...
  const canSpin = useCallback(() => {
    ...
    if (inertiaRAF.current) return false;
    if (focusedElRef.current) return false; // imagen abierta
    if (rootRef.current?.getAttribute('data-enlarging') === 'true') return false;
    return true;
  }, [autoSpin]);
  ...
  useEffect(() => {
    const step = (t) => {
      ...
      if (canSpin()) {
        const y = wrapAngleSigned(rotationRef.current.y - spinDegPerSec * dt); // izquierda
  ...
  useEffect(() => {
    ...
    // para touch: pausa al tocar
    const onTouchStart = () => { hoverRef.current = true; };
  ...
  const cssStyles = `
    .sphere-root { --radius: 520px; --viewer-pad: 72px; --circ: calc(var(--radius) * 3.14);
      --rot-y: calc((360deg / var(--segments-x)) / 2); --rot-x: calc((360deg / var(--segments-y)) / 2);
      --item-width: calc(var(--circ) / var(--segments-x)); --item-height: calc(var(--circ) / var(--segments-y)); }
    .sphere-root * { box-sizing: border-box; }
    .sphere, .sphere-item, .item__image { transform-style: preserve-3d; }
    .stage { width:100%; height:100%; display:grid; place-items:center; position:absolute; inset:0; margin:auto;
    ...
    .sphere-item { width: calc(var(--item-width) * var(--item-size-x)); height: calc(var(--item-height) * var(--item-size-y));
    ...
    .item__image { position:absolute; inset:10px; border-radius: var(--tile-radius, 12px); overflow:hidden; cursor:pointer;
      backface-visibility:hidden; -webkit-backface-visibility:hidden; transition:transform 300ms; pointer-events:auto;
